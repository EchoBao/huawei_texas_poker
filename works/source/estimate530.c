...
int InitOrder(int CardNum)//排序之前的初始化
{
	...
	else if(CardNum==8)//记录桌面上的五张
	{
		...
		DeskInOrder[3].Point = 0;
		DeskInOrder[3].Color = 0;
...
{
	if((MyID.JettonLeft >= CurrentPrice)&&(CurrentPrice))
		return 1;
	else 
		return 0;
}
...
int MakeDecision(void)//做出操作决策的函数,返回2表示check,返回3表示call,返回4表示raise,返回5表示all_in,返回6表示fold
{
	...
	if(GameStatus==1)//在手牌阶段
	{
		...
		if(Power<16)
			return 6;//牌力小于10弃牌
		else if((Power>=16)&&(Power<=20))//参考以下总注比
			{
				if(CurrentPrice>=(MyGame.Players*40)+80)//如果当前需要加注超过500就弃牌
					return 6;//fold		
				return 3;
			}
		else if((Power>20)&&(Power<=33))
			{
