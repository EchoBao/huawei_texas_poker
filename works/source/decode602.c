...
int DecodeFrame(const char* buf,int type)//解析包函数，type表示包的类型，1表示解析座次消息
{
	...
	if(4 == type)//解析询问消息
	{
		...
		if((start=strstr(buf,"inquire/"))&&(end=strstr(buf,"total")))
		{
			...
			if(GamerInitFlag)
			{
				MyGamers[0].ID = MyUpper.ID;
				MyGamers[0].OnLine = MyUpper.OnLine;
				MyGamers[0].OperNum++;
				MyGamers[0].LastAction = MyUpper.LastAction;
				if(MyGamers[0].LastAction==6)
					MyGamers[0].FoldNum++;
			}
			else 
			{
				for(i=0;i<8;i++)
				{
					if(MyGamers[i].ID == MyUpper.ID)
						break;
				}
				MyGamers[i].ID = MyUpper.ID;
				MyGamers[i].OnLine = MyUpper.OnLine;
				MyGamers[i].OperNum++;
				MyGamers[i].LastAction = MyUpper.LastAction;
				if(MyGamers[i].LastAction==6)
					MyGamers[0].FoldNum++;
			...
			printf("%d,%d\n",MyGame.FirstID.OperNum,MyGame.FirstID.FoldNum);	
